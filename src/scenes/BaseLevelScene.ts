...
export const START = { x: 90, y: 100 }
export const CUP = { x: 780, y: 540 }
...
function distanceToSegment(
  ...
  if (lengthSq === 0) return Phaser.Math.Distance.Between(p.x, p.y, a.x, a.y)
...
export abstract class BaseLevelScene extends Phaser.Scene {
  ...
  private dragOrigin: { p1?: Phaser.Math.Vector2; p2?: Phaser.Math.Vector2; pos?: Phaser.Math.Vector2 } = {}
  ...
  create() {
    ...
    this.add.text(20, 16, this.levelLabel, { fontSize: '20px', color: '#0b1f33' }).setDepth(10)

    this.instructionsText = this.add
      .text(20, 40, '', { fontSize: '14px', color: '#0b1f33' })
      .setDepth(10)
    ...
    this.messageText = this.add
      .text(WORLD_WIDTH / 2, 260, '', {
        ...
        padding: { x: 20, y: 12 },
        align: 'center',
        wordWrap: { width: WORLD_WIDTH - 120, useAdvancedWrap: true },
      })
      .setOrigin(0.5)
      .setDepth(20)
      .setVisible(false)
    ...
    this.matter.world.on('collisionstart', (event: MatterJS.IEventCollision<MatterJS.Engine>) => {
      for (const pair of event.pairs) {
        ...
        if (labels.includes('ball') && labels.includes('spring') && this.state === 'rolling') {
          this.matter.body.setVelocity(this.ball, { x: this.ball.velocity.x, y: -SPRING_LAUNCH_SPEED })
        }
      }
    })
  }
  ...
  private buildCup() {
    ...
    const walls: Array<[Phaser.Math.Vector2, Phaser.Math.Vector2]> = [
      [new Phaser.Math.Vector2(CUP.x - bottomWidth / 2, bottomY), new Phaser.Math.Vector2(CUP.x - topWidth / 2, topY)],
      [new Phaser.Math.Vector2(CUP.x + bottomWidth / 2, bottomY), new Phaser.Math.Vector2(CUP.x + topWidth / 2, topY)],
    ]
  ...
  private onPointerDown(pointer: Phaser.Input.Pointer) {
    if (this.state !== 'placing') return
    if (pointer.y < PLACEMENT_AREA_TOP) return
    this.messageText.setVisible(false)
    ...
    if (hit) {
      ...
      this.dragOrigin = hit.type === 'board' ? { p1: hit.p1.clone(), p2: hit.p2.clone() } : { pos: hit.pos.clone() }
  ...
  private finalizeBoard(p1: Phaser.Math.Vector2, p2: Phaser.Math.Vector2) {
    ...
    if (length < BOARD_MIN_LENGTH) return
    ...
    this.placedElements.push({ type: 'board', p1: p1.clone(), p2: p2.clone(), body, view })
  }
  ...
  private placeSpring(pos: Phaser.Math.Vector2) {
    ...
    this.placedElements.push({ type: 'spring', pos: pos.clone(), body, view })
  }
  ...
  private findElementAt(pos: Phaser.Math.Vector2): PlacedElement | null {
    for (let i = this.placedElements.length - 1; i >= 0; i--) {
      ...
      if (el.type === 'board') {
        if (distanceToSegment(pos, el.p1, el.p2) <= BOARD_THICKNESS / 2 + SELECT_HIT_MARGIN) return el
      } else {
  ...
  private setElementHighlight(el: PlacedElement, on: boolean) {
    if (el.type === 'board') {
      ...
      if (on) this.showHandlesFor(el)
      else this.clearHandles()
    } else {
  ...
  private clearHandles() {
    for (const handle of this.selectionHandles) handle.destroy()
    this.selectionHandles = []
  }

  private findHandleAt(pos: Phaser.Math.Vector2, el: PlacedBoard): 'p1' | 'p2' | null {
    if (Phaser.Math.Distance.Between(pos.x, pos.y, el.p1.x, el.p1.y) <= HANDLE_HIT_RADIUS) return 'p1'
    if (Phaser.Math.Distance.Between(pos.x, pos.y, el.p2.x, el.p2.y) <= HANDLE_HIT_RADIUS) return 'p2'
    return null
  }

  private dragBoardHandle(el: PlacedBoard, handle: 'p1' | 'p2', pos: Phaser.Math.Vector2) {
    if (handle === 'p1') el.p1 = pos.clone()
    else el.p2 = pos.clone()
  ...
  private clearSelection() {
    if (this.selectedElement) this.setElementHighlight(this.selectedElement, false)
  ...
  private dragSelectedElement(pos: Phaser.Math.Vector2) {
    ...
    if (el.type === 'board') {
      el.p1 = this.dragOrigin.p1!.clone().add({ x: deltaX, y: deltaY })
      el.p2 = this.dragOrigin.p2!.clone().add({ x: deltaX, y: deltaY })
      const midX = (el.p1.x + el.p2.x) / 2
      const midY = (el.p1.y + el.p2.y) / 2
      const angle = Phaser.Math.Angle.Between(el.p1.x, el.p1.y, el.p2.x, el.p2.y)
      el.view.setPosition(midX, midY).setRotation(angle)
      this.matter.body.setPosition(el.body, { x: midX, y: midY })
      this.matter.body.setAngle(el.body, angle)
      this.updateHandlePositions(el)
    } else {
      el.pos = this.dragOrigin.pos!.clone().add({ x: deltaX, y: deltaY })
      el.view.setPosition(el.pos.x, el.pos.y)
      this.matter.body.setPosition(el.body, { x: el.pos.x, y: el.pos.y })
    }
  }
  ...
  private onMiss() {
    ...
    this.state = 'placing'
    this.matter.body.setStatic(this.ball, true)
    this.matter.body.setPosition(this.ball, { x: START.x, y: START.y })
